...
\section{Maxwell's relations}
    ...
    Integrability condition can be written as Jacobian determinant in the following way 
    \begin{equation*}
    ...
    \begin{equation*}
        \pdv{(T, S, N)}{(p, V, N)} = 1 ~, \quad \pdv{(T, S, V)}{(\mu, N, V)} = - 1 ~, \quad \pdv{(p, V, S)}{(\mu, N, S)} = 1 ~.
    \end{equation*}
    \begin{proof}
        ...
        \begin{equation*}
            \pdv{T}{N} \Big \vert_{S, V} = \pdv{\mu}{S} \Big \vert_{N, V} \rightarrow \pdv{(T, S, V)}{(N, S, V)} = \pdv{(\mu, N, V)}{(S, N, V)} = - \pdv{(\mu, N, V)}{(N, S, V)} ~,
        \end{equation*} 
        hence, inverting the right-handed side, we find
        \begin{equation*}
            - 1 = \pdv{(T, S, V)}{(N, S, V)} \pdv{(\mu, N, V)}{(N, S, V)}^{-1} = \pdv{(T, S, V)}{(N, S, V)} \pdv{(N, S, V)}{(\mu, N, V)} = \pdv{(T, S, V)}{(\mu, N, V)} ~.
        \end{equation*} 
    ...
    The geometrical interpretation is that coordinate transformations, which mean that we changed into a different chart of independent variables, preserve volumes. 
